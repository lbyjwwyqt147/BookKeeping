...
/***
 * 文件名称: RoleModuleController.java
 * 文件描述: 角色模块Controller
 * 公 司: 
 * 内容摘要: 
 * 其他说明:
 ...
 */
@Controller
@RequestMapping("/bookkeeping/roleModule")
public class RoleModuleController {
	...
	public void saveInfo(String roleId, String roleCode, String moduleId,
			String moduleCode,HttpServletRequest request,HttpServletResponse response){
		String resultJson = roleModuleService.saveInfo(roleId, roleCode, moduleId, moduleCode);
	...
	public void deletes(String id,HttpServletRequest request,HttpServletResponse response){
		...
		try {
			String[] ids = id.split(",");
			AtomicInteger count =  new AtomicInteger(roleModuleService.deletes(ids));
		    if(count.get() > 0){
		    	success.set(true);
	...
	public void findList(Integer pageNum,Integer limit,HttpServletRequest request,HttpServletResponse response){
		...
		try {
			...
			CopyOnWriteArrayList<TCoreRoleModule> roleFunsList = roleModuleService.findList(paramsMap);
			//获取分页列表
			PageBean<TCoreRoleModule> pageList = new PageBean<TCoreRoleModule>(page.getResult(), pageNum);
