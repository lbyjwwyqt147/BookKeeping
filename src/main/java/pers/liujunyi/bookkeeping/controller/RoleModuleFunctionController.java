...
/***
 * 文件名称: RoleModuleFunctionController.java
 * 文件描述: 角色资源权限(功能)Controller
 * 公 司: 
 * 内容摘要: 
 * 其他说明:
 * 完成日期:2016年11月23日 
 ...
 */
@Controller
@RequestMapping("/bookkeeping/roleModuleFunction")
public class RoleModuleFunctionController {
	...
	public void saveInfo(String roleId, String roleCode, String functionId,String moduleId,HttpServletRequest request,HttpServletResponse response){
		String resultJson = roleModuleFunctionService.saveInfo(roleId, roleCode,moduleId, functionId);
	...
	public void deletes(String id,HttpServletRequest request,HttpServletResponse response){
		...
		try {
			String[] ids = id.split(",");
			AtomicInteger count =  new AtomicInteger(roleModuleFunctionService.deletes(ids));
		    if(count.get() > 0){
		    	success.set(true);
	...
	public void findList(Integer pageNum,Integer limit,HttpServletRequest request,HttpServletResponse response){
		...
		try {
			...
			CopyOnWriteArrayList<TCoreRoleModuleFunction> roleFunsList = roleModuleFunctionService.findList(paramsMap);
			//获取分页列表
			PageBean<TCoreRoleModuleFunction> pageList = new PageBean<TCoreRoleModuleFunction>(page.getResult(), pageNum);
